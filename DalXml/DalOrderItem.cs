...
internal class DalOrderItem : IOrderItem
{
    ...
    static DalOrderItem()
    {
        if (!Directory.Exists(OrderItemFilePath))
            Directory.CreateDirectory(OrderItemFilePath);
    }

    static string OrderItemFilePath = @"C:\Users\שרה\source\repos\dotNet5783_-5840_9047\xml\OrderItem.xml";

    public DalOrderItem()
    {
        if (!File.Exists( OrderItemFilePath))
            XMLTools.SaveListToXMLSerializer<DO.OrderItem?>(DataSource.m_listOrderItems, OrderItemFilePath);
    }


    public int Add(OrderItem? OI)
    {
        List<DO.OrderItem?> OrderItemList = XMLTools.LoadListFromXMLSerializer<DO.OrderItem?>(OrderItemFilePath);
        OrderItem oi = OI ?? throw new ArgumentNull();
        oi.m_ID = Config.getOrderItemId();
        OrderItemList.Add(oi);

        XMLTools.SaveListToXMLSerializer<DO.OrderItem?>(OrderItemList, OrderItemFilePath);
        return oi.m_ID;
    }
